...
test("bridges compiler fixes and explicit refactors into VS Code workspace edits", () => {
  assert.match(extension, /registerCodeActionsProvider/);
  assert.match(extension, /textDocument\/codeAction/);
  assert.match(extension, /vscode\.CodeActionKind\.QuickFix/);
  assert.match(extension, /vscode\.CodeActionKind\.RefactorRewrite/);
  assert.match(extension, /"refactor\.rewrite": vscode\.CodeActionKind\.RefactorRewrite/);
...
test("keeps server-owned callable generation actions generic across documents", () => {
  assert.match(extension, /result\.edit = toWorkspaceEdit\(action\.edit\)/);
  assert.match(extension, /for \(const \[uri, changes\] of Object\.entries\(workspaceEdit\.changes \?\? \{\}\)\)/);
