...
The pinned compiler supplies Stage 30f debug, native, and PHP compatibility
closure execution plus
function-value ownership, lifetime, escape, diagnostic, fix, and hover facts. The
server only adapts those structured facts to LSP. Ordinary editor analysis is
target-neutral and does not publish `E0641` for valid closures. PHP remains a
secondary compatibility backend with independent limitations; Stage 30g List
algorithms are next, and Stage 30 remains incomplete.
